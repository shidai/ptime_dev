...
void loadPrimaryHeader(fitsfile *fp,dSet *data)
{
  ...
  fits_read_key(fp,TSTRING,(char *)"OBS_MODE",data->phead.obsMode,NULL,&status);
  fits_read_key(fp,TSTRING,(char *)"SRC_NAME",data->phead.source,NULL,&status);
  if (status)
    {
      fits_report_error(stderr,status);
      exit(1);
  ...
  if (status)
    ...
    {
      data->subintTable=1;
      fits_read_key(fp,TINT,(char *)"NAXIS2",&(data->phead.nsub),NULL,&status);
      if (status)
	{
      ...
      printf("Read naxis 2\n");
      fits_read_key(fp,TINT,(char *)"NCHAN",&(data->phead.nchan),NULL,&status);
      if (status)
	{
      ...
      /*      fits_read_key(fp,TFLOAT,(char *)"ZERO_OFF",&(data->phead.zeroOff),NULL,&status);
      if (status)
	{
	  ...
	  printf("Complete reading zero_off\n");
       ...
       printf("COMMENTED OUT READING ZERO_OFF AND NBITS IN PFITS.C -- PUT BACK -- PROBLEM WITH CASPSR FILES\n");
            fits_read_key(fp,TINT,(char *)"NBITS",&(data->phead.nbits),NULL,&status);
      if (status)
	{
      ...
      fits_read_key(fp,TINT,(char *)"NPOL",&(data->phead.npol),NULL,&status);
      if (status)
	{
	  printf("Reading npol\n");
	  fits_report_error(stderr,status);
      ...
      fits_read_key(fp,TINT,(char *)"NSBLK",&(data->phead.nsblk),NULL,&status);
      if (status)
	{
      ...
      fits_read_key(fp,TINT,(char *)"NBIN",&(data->phead.nbin),NULL,&status);
      if (status)
	{
	  printf("Reading nbin\n");
	  fits_report_error(stderr,status);
      ...
      fits_read_key(fp,TFLOAT,(char *)"CHAN_BW",&(data->phead.chanbw),NULL,&status);
      if (data->phead.chanbw < 0 && data->phead.bw > 0)
	data->phead.bw*=-1;
      
      fits_read_key(fp,TFLOAT,(char *)"TBIN",&(data->phead.tsamp),NULL,&status);
...
//  float bpass[data->phead.nchan*2];
  ...
  fits_movnam_hdu(fp,BINARY_TBL,(char *)"BANDPASS",1,&status);
  if (status) {
...
int extractPolData(fitsfile *fp,dSet *data,int pol,float *arr,float t1,float t2)
{
  ...
  printf("Have samples (%ld,%ld) (%ld,%ld)\n",sub_1,samp_1,sub_2,samp_2);
  fits_movnam_hdu(fp,BINARY_TBL,(char *)"SUBINT",1,&status);
  if (status) {
    printf("Unable to move to subint table in FITS file\n");
    exit(1);
  ...
  for (s=sub_1;s<=sub_2;s++)
	...
	r2 = nsblk;
      printf("Reading from %ld to %ld for subint %ld\n",r1,r2,s);
...
int extractData(fitsfile *fp,dSet *data,long s1,long s2,float *mean,float *min,float *max,long maxVal,int *nsmooth,float dm)
{
  ...
  nsblk = data->phead.nsblk;
  printf("Finding sample 1 %d %d\n",(int)s1,(int)s2);
  findPosSample(data,s1,&sub_1,&samp_1);
  printf("Finding sample 2\n");
  findPosSample(data,s2,&sub_2,&samp_2);
    printf("Here with %d %d %d %d\n",sub_1,samp_1,sub_2,samp_2);
  // Go to the subint table
  ...
  for (i=0;i<nchan;i++)
      ...
      for (j=0;j<nsampDM;j++)
	{
	  ...
	  cde_byte[j][i] = (unsigned int)k;
	  printf("Setting bit\n");
	  cde_bit[j][i]  = (unsigned char)(i%8);
	  printf("Done set\n");
	}
    }
...
// Dedisperse all
// Routine to dedisperse at a large number of DM steps simultaneously and 
// return timeseries of the dedispersed data. 
// This can also process multiple files (e.g., beams) simultaneously
//
// Note: this can include negative DM values
//
//
// Inputs:
// nDM: number of dispersion trials
// *dm: list of DM values (must be in ascending order)
// **res: resulting time series of dedispersed time series
// nFiles: number of data files
int dedisperse_all(int nDM,float *dm,float **res,int nFiles,fitsfile *fp,dSet *data)
{
  long sub_1,samp_1;
  long sub_2,samp_2;
  int  s,r1,r2;
  ...
  s1 = 0;
  s2 = data->phead.nsblk*data->phead.nsub;
  ...
  // Number of extra samples required to process 
  // to de-disperse across the band
  tDM = fabs(4.15e-3*dm[nDM-1]*(pow((f0)/1000.0,-2)-pow((f0-fabs(bw))/1000.0,-2)));
  nsampDM = ceil(tDM/data->phead.tsamp);
  ...
  for (i=0;i<nsampDM;i++)
    {
      cde_byte[i] = (unsigned int **)malloc(sizeof(unsigned int *)*nchan);
      cde_bit[i] = (unsigned char **)malloc(sizeof(unsigned char *)*nchan);
      for (d=0;d<nchan;d++)
	{
	  cde_byte[i][d] = (unsigned int *)malloc(sizeof(unsigned int)*nDM);
	  cde_bit[i][d] = (unsigned char *)malloc(sizeof(unsigned char)*nDM);
	}
    }
  printf("Allocated memory\n");
  ...
  for (i=0;i<nchan;i++)
    {
      for (d=0;d<nDM;d++)
	{
	  tdelay = 4.15e-3*dm[d]*(pow(f0/1000.0,-2)-pow((f0-fabs(chanbw)*i)/1000.0,-2));
	  cdelay = nint(-tdelay/data->phead.tsamp);
	  //      printf("Have %g %d\n",tdelay,cdelay);
	  for (j=0;j<nsampDM;j++)
	    {
		...
		for (j=0;j<nchan;j++)
		  {
		    //	      bitCount += extractBit(cval[cde_byte[t][j]],cde_bit[t][j]);
		    // ONLY 1-BIT DATA
		    bitCount += extractBit(cval[cde_byte[t][j][d]],j%8);
		    
		    //	      bitCount+=cde_byte[t][j]+cde_bit[t][j];
		  }
	      }
	      res[d][count] = bitCount;
	    }
	  t++;
	  pos++;
	  count++;
