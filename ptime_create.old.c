...
void doFit(float *xval,float *data,int nbin,int comp);
...
void help()
{
  printf("Left mouse button - add component\n");
  printf("f - fit components\n");
  printf("q - quit\n");
  printf("z - zoom\n");
}
...
int main(int argc,char *argv[])
{
  ...
  char fname[128];
  //  float data[MAX_BIN];
  float residual[MAX_BIN];
  float xval[MAX_BIN];
  float comp[MAX_BIN];
  float template[MAX_BIN];
  float miny0,maxy0;
  int nbin;
  FILE *fin,*fout;
  float minx,maxx,miny,maxy;
  float minyRes,maxyRes;
  float mouseX,mouseY;
  char key;
  ...
  printf("Routine to fit Von Mises functions to a data set\n");
  help();
  for (i=0;i<argc;i++)
    {
      if (strcmp(argv[i],"-f")==0)
	strcpy(fname,argv[++i]);
      else if (strcmp(argv[i],"-l")==0) // Load template file
	{
  ...
  fx = (float *)malloc(sizeof(float)*data->phead.nbin*2);
  fy = (float *)malloc(sizeof(float)*data->phead.nbin*2);
  ...
  nbin = data->phead.nbin;
  extractFoldData(fp,data,-1,fx,fy,freq_y,time_y,bpass, 0);
  //  for (i=0;i<data->phead.nbin*data->phead.nchan;i++)
  //    {
  //      printf("Have: %d %g\n",i,freq_y[i]);
  //    }
  // Must remove baseline
  // MUST DO THIS PROPERLY
  {
    double bl=0;
    for (i=700;i<1000;i++)
      bl+=freq_y[i+chan*nbin];
    bl/=(1000.0-700.0);
    for (i=0;i<nbin;i++)
      fy[i]=freq_y[i+chan*nbin]-bl;
  }
  closeFitsFile(fp);
  //  printf("Complete extraction\n");
  //    for (i=0;i<nbin;i++)
  //      printf("Have %g %g\n",fx[i],fy[i]);
  ...
  for (i=0;i<nbin;i++)
      ...
      if (i==0)
	{
      ...
      else if (miny0 > data[i])
	miny0 = data[i];
      else if (maxy0 < data[i])
	maxy0 = data[i];
      data[nbin+i] = data[i];
      xval[i] = i;
      xval[nbin+i] = nbin+i;
  ...
  for (i=0;i<nbin;i++)
    {
      if (i==0){miny0 = maxy0 = fy[i];}
      else if (miny0 > fy[i]) miny0 = fy[i];
      else if (maxy0 < fy[i]) maxy0 = fy[i];
      fx[i+nbin] = i+nbin;
      fy[i+nbin] = fy[i];
    }
  cpgbeg(0,"/xs",1,1);
  ...
  printf("Starting\n");
  do {
    sprintf(plotTitle,"channel: %d",chan);
    ...
    printf("Drawing the values\n");
    cpgbin(2*nbin,fx,fy,1);
    cpglab("","",plotTitle);
    // Draw the components
    printf("\n");
    for (i=0;i<nComp[chan];i++)
      {
	printf("Component %d, height = %g, concentration = %g, centroid = %g\n",i,height[chan][i],concentration[chan][i],centroid[chan][i]);
	for (j=0;j<nbin;j++)
	  {
	    comp[j] = height[chan][i]*exp(concentration[chan][i]*(cos((j-(centroid[chan][i]))/(double)nbin*2*M_PI)-1));
	    comp[nbin+j] = comp[j];
	    if (i==0)
	      {
		template[j] = comp[j];
		template[j+nbin] = comp[j+nbin];
	      }
	    else
	      {
		template[j] += comp[j];
	...
	cpgsci(7); cpgsls(4);
	cpgline(2*nbin,fx,comp); cpgsls(1);
      }
    for (i=0;i<2*nbin;i++)
      {
	residual[i] = fy[i]-template[i];
	if (i==0)
	  {
	    minyRes=maxyRes = residual[i];
	  }
	else
	  {
	    if (minyRes > residual[i]) minyRes = residual[i];
	    if (maxyRes < residual[i]) maxyRes = residual[i];
	  }
      }
    cpgsci(3);
    cpgline(2*nbin,fx,template);
    cpgsci(1);
    cpgsvp(0.1,0.9,0.1,0.5);
    cpgswin(minx,maxx,minyRes,maxyRes);
    cpgbox("BCTSN",0.0,0,"BCTSN",0.0,0);
    cpglab("Bin number","","");
    cpgsci(8);
    cpgbin(2*nbin,fx,residual,1);
    cpgsci(1);

    // Check the cursor
    cpgcurs(&mouseX,&mouseY,&key);
    if (key=='A') // Define a component
      {
	float mx2,my2,mx3,my3,mx4,my4;
	cpgband(4,0,mouseX,mouseY,&mx3,&my3,&key);	
	if (key!='q'){
	  cpgband(5,0,mouseX,mouseY,&mx4,&my4,&key);	
	  if (key!='q') {
	    if (nComp[chan] == MAX_COMP-1)
	      {
		printf("Too many components\n");
	      }
	    else
	      {
		centroid[chan][nComp[chan]] = mouseX;
		concentration[chan][nComp[chan]] = nbin*nbin*0.25/pow(fabs(mx3-mouseX),2);
		height[chan][nComp[chan]] = my4;
		nComp[chan]++;
	      }
	  }
	}
    ...
    else if (key=='c') // Set channel
      {
	int ochan = chan;
	printf("Please enter channel number (from %d to %d): ",0,data->phead.nchan-1);
	scanf("%d",&chan);
	if (nComp[chan]==0) {
	  // Copy across the current template
	  nComp[chan] = nComp[ochan];
	  for (i=0;i<nComp[chan];i++)
	    {
	      centroid[chan][i] = centroid[ochan][i];
	      concentration[chan][i] = concentration[ochan][i];
	      height[chan][i] = height[ochan][i];
	    }
	}
	// MUST DO THIS PROPERLY
	{
	...
	for (i=0;i<nbin;i++)
	  {
	    if (i==0){miny0 = maxy0 = fy[i];}
	    else if (miny0 > freq_y[i]) miny0 = freq_y[i];
	    else if (maxy0 < freq_y[i]) maxy0 = freq_y[i];
	  }
	miny = miny0;
	maxy = maxy0;
      }
	...
	maxy = maxy0;
      ...
      {
	float mx2,my2;
	cpgband(4,0,mouseX,mouseY,&mx2,&my2,&key);
	if (mx2 < mouseX) minx = mx2;
	else minx = mouseX;
	if (mx2 > mouseX) maxx = mx2;
	else maxx = mouseX;

	/*	if (my2 < mouseY) miny = my2;
	else miny = mouseY;
	if (my2 > mouseY) maxy = my2;
	else miny = mouseY; */
      }
    else if (key=='Z')
      {
	float mx2,my2;
	cpgband(2,0,mouseX,mouseY,&mx2,&my2,&key);
	...
	printf("Using %g %g\n",mouseY,my2);
      ...
      {
	doFit(fx,fy,nbin,chan);
      }
  } while (key != 'q');

  // Saving the components file
  {
    double f1,f2;

    fout = fopen("templatesComp.dat","w");
    fprintf(fout,"TMPL_VERSION 1.0\n");
    fprintf(fout,"ORIGINAL_FILE %s\n",fname);
    fprintf(fout,"STOKES I\n");
    for (j=0;j<data->phead.nchan;j++)
      {
	// Make this nicer and deal with positive BWs
	f2 = data->phead.freq - data->phead.bw/2.0 - j*data->phead.nchan;
	f1 = data->phead.freq - data->phead.bw/2.0 - (j+1)*data->phead.nchan;
	fprintf(fout,"\nFREQUENCY_RANGE %g %g\n",f1,f2);
	fprintf(fout,"NCOMP %d\n",nComp[j]);
	for (i=0;i<nComp[j];i++)
	  fprintf(fout,"COMP%d %.5f %.5f %.5f\n",i,height[j][i],concentration[j][i],centroid[j][i]);
      }
    fclose(fout);
    printf("Result saved to templatesComp.dat\n");
  }
  cpgend();
...
void doFit(float *xval,float *data,int nbin,int chan)
{
  lm_status_struct status;
  lm_control_struct control = lm_control_double;
  control.printflags = 0; // monitor status (+1) and parameters (+2)
  double pval[nComp[chan]*3],origChisq;
  double datX[nbin],datY[nbin];
  int i,j;
  double chisq,yval,x,rot;
  double offset_rotation;     
  double offset_overallScale; 
  double offset_baseline;

  printf("Doing the fit\n");
  for (i=0;i<nbin;i++)
    {
      datX[i] = (double)xval[i];
      datY[i] = (float)data[i];
      //      printf("datY[i] = %d %g\n",i,datY[i]);
    }
  for (i=0;i<nComp[chan];i++)
    {
      pval[3*i+0] = height[chan][i]; // Height
      pval[3*i+1] = concentration[chan][i]; // Concentration
      pval[3*i+2] = centroid[chan][i]; // Centroid
    }
  global_nbin = nbin;
  global_chan = chan;
  lmcurve_fit(3*nComp[chan],pval,nbin,datX,datY,nonlinearFunc,&control,&status);
  for (i=0;i<nComp[chan];i++)
    printf("Resulting values = %g %g %g\n",pval[3*i+0],pval[3*i+1],pval[3*i+2]);
  for (i=0;i<nComp[chan];i++)
    {
      height[chan][i]        = pval[3*i+0];
      concentration[chan][i] = pval[3*i+1];
      centroid[chan][i]      = pval[3*i+2];
    }
  //  std->height[0]           = pval[3];
  //  std->height[1]           = pval[4];

  //  printf("Results %g %g %g %g\n",pval[0],pval[1],pval[2],pval[3]);
  ...
  do { 
    chisq=0.0;
    for (i=0;i<obs->nbin;i++)
      {
	yval=0.0;
	x = (double)(i+0.5)/(double)obs->nbin*2*M_PI+rot/100.0;
	yval = evaluateStandard(std,x,-1);      
	chisq+=pow(datY[i]-yval,2);
      }
    if (origChisq==-1) origChisq = chisq;
    printf("Fit chisq = %g %g %g\n",rot,chisq,origChisq);
    rot+=1.0;
    } while (fabs(chisq  - origChisq) < 1); */
}
...
double nonlinearFunc( double x, const double *par)
{
  ...
  for (k=0;k<nComp[global_chan];k++)
    result += par[3*k+0]*exp(par[3*k+1]*(cos((x-(par[3*k+2]))/(double)global_nbin*2*M_PI)-1));
